...
export default function App(){
 ...
 const authFetch=async(path)=>{const r=await fetch(API+path,{headers:{Authorization:'Bearer '+tok}});
  if(r.status===401||r.status===403){out();throw new Error('session_expired')}
  if(!r.ok)throw new Error('request_failed');return r.json()};
 useEffect(()=>{if(!tok)return;const es=new EventSource(API+'/api/live?token='+tok);
  es.onmessage=e=>{const c=JSON.parse(e.data);setFeed(f=>[c,...f].slice(0,8));setChecks(x=>[c,...(x||[])].slice(0,100));
   ...
   authFetch('/api/stats').then(setStats).catch(()=>{})};es.onerror=()=>{};return()=>es.close()},[tok]);
 useEffect(()=>{if(!tok)return;let live=true;
  Promise.all([authFetch('/api/stats'),authFetch('/api/checks')]).then(([s,c])=>{if(live){setStats(s);setChecks(Array.isArray(c)?c:[])}}).catch(()=>{});
 ...
 return<div>{Bar}<nav className="tabs">{t.nav.map((n,i)=><button key={n} className={page===i?'on':''} onClick={()=>setPage(i)}>{n}</button>)}</nav>{alert&&<div className="alert" role="alert" onClick={()=>setAlert(null)}>🚨 {t.f[alert.flags?.[0]]||t.s.fraud} · <b>{alert.plate}</b> · {alert.checkpoint}</div>}
  ...
  <div className="grid"><div className="card"><h3>{t.week}</h3><div className="bars">{daily.map((d,i)=><div key={i} className="col" title={d.n}>
    <div className="b" style={{height:d.n/mx*100+'%'}}><div className="bf" style={{height:Math.min(100,d.fraud/d.n*400)+'%'}}/></div></div>)}</div></div>
   <div className="card"><h3>{t.flags}</h3>{flags.map(x=><div key={x._id} className="row"><span>{t.f[x._id]||x._id}</span><div className="tr"><div style={{width:x.n/fm*100+'%'}}/></div><em>{x.n}</em></div>)}</div>
   <div className="card"><h3>{t.cps}</h3>{cps.map(x=><div key={x._id} className="row"><span>{x._id}</span><em>{x.n.toLocaleString()}</em></div>)}</div></div>
