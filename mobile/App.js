...
import {View,Text,TextInput,TouchableOpacity,Image,ScrollView,useColorScheme,StyleSheet,ActivityIndicator,Alert} from 'react-native';
...
const T={en:{scan:'Scan',hist:'History',go:'Verify',owner:'Owner',make:'Vehicle',ins:'Insurance',insp:'Inspection',seen:'Previous checks',qr:'Scan QR code',off:'Offline – using cache',on:'Online',pend:'pending sync',login:'Sign in',email:'Email',pw:'Password',
...
fr:{scan:'Scanner',hist:'Historique',go:'Vérifier',owner:'Propriétaire',make:'Véhicule',ins:'Assurance',insp:'Visite technique',seen:'Contrôles précédents',qr:'Scanner le QR code',off:'Hors ligne – cache utilisé',on:'En ligne',pend:'en attente de synchro',login:'Connexion',email:'E-mail',pw:'Mot de passe',
...
export default function App(){
 ...
 const ref=useRef(null),[mode,setMode]=useState('qr'),[cp,setCp]=useState(CHECKPOINTS[0]),t=T[lang],[tab,setTab]=useState('scan'),[tok,setTok]=useState(null),[f,setF]=useState({email:'',password:''}),[plate,setPlate]=useState(''),[res,setRes]=useState(null),
 ...
 async function login(){const r=await fetch(API+'/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(f)}).catch(()=>null);const d=r&&await r.json();if(d?.token){await AsyncStorage.setItem('tok',d.token);setTok(d.token)}}
 async function verify(p){if(!norm(p))return;setBusy(true);const at=new Date().toISOString();let r;
  ...
  const e={plate:norm(p),status:r.status,flags:r.flags,at,vehicle:r.vehicle,seen:hist.filter(h=>h.plate===norm(p)).length};setRes(e);save('hist',[e,...hist].slice(0,50),setHist);setBusy(false);setCam(false)}
 ...
 const Top=<View style={{flexDirection:'row',alignItems:'center',padding:14,paddingTop:48,backgroundColor:c.card,gap:10}}><Image source={require('./assets/logo.png')} style={{width:36,height:36}} resizeMode="contain"/>
 ...
 if(!tok)return<View style={{flex:1,backgroundColor:c.bg}}>{Top}<View style={{padding:24,gap:6}}><Image source={require('./assets/logo.png')} style={{width:'100%',height:220}} resizeMode="contain"/>
  <TextInput style={st.in} placeholder={t.email} placeholderTextColor={c.mu} autoCapitalize="none" onChangeText={v=>setF({...f,email:v})}/>
  <TextInput style={st.in} placeholder={t.pw} placeholderTextColor={c.mu} secureTextEntry onChangeText={v=>setF({...f,password:v})}/>
  <TouchableOpacity style={st.btn} onPress={login}><Text style={{color:'#fff',fontWeight:'700'}}>{t.login}</Text></TouchableOpacity></View></View>;
 return<View style={{flex:1,backgroundColor:c.bg}}>{Top}
  ...
  {tab==='scan'?<>
   ...
   :<View style={{flexDirection:'row',gap:10,marginBottom:14}}>{[['qr','📷 '+t.qr],['ocr','🔍 '+t.mode]].map(([m,l])=><TouchableOpacity key={m} style={[st.btn,{flex:1}]} onPress={async()=>{if(!perm?.granted)await ask();setMode(m);setCam(true)}}><Text style={{color:'#fff',fontWeight:'700',textAlign:'center'}}>{l}</Text></TouchableOpacity>)}</View>}
   ...
   {res&&<View style={[st.card,{borderColor:COL[res.status],borderWidth:2}]}><Text style={{fontSize:44,textAlign:'center'}}>{res.status==='valid'?'✅':res.status==='warning'?'⚠️':'⛔'}</Text>
  ...
  <View style={{flexDirection:'row',backgroundColor:c.card,paddingBottom:24}}>{['scan','hist'].map(k=><TouchableOpacity key={k} style={{flex:1,padding:14,alignItems:'center'}} onPress={()=>setTab(k)}><Text style={{color:tab===k?c.pr:c.mu,fontWeight:'700'}}>{t[k]}</Text></TouchableOpacity>)}</View></View>}
